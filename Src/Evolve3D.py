...
class Gene:
    def _init_(self, source_type, source_num, sink_type, sink_num, weight, source_pos=None, sink_pos=None):
...
class Individual:
    def _init_(self, genome):
        self.genome = genome
        self.fitness = 0
        self.brain = MLPRegressor(hidden_layer_sizes=(100, 50), max_iter=500)
        self.memory = []
        self.energy = 100
    ...
    def learn(self, experiences):
        if experiences:
            X, y = zip(*experiences)
            self.brain.partial_fit(np.array(X), np.array(y))
    def evaluate_fitness(self):
        self.fitness = self.energy - sum(g.weight for g in self.genome.genes)
    def mutate(self):
...
class Environment:
    def _init_(self):
        self.resources = [random.randint(10, 100) for _ in range(5)]
    def get_observation(self):
        return np.array(self.resources)
    def update(self):
        self.resources = [random.randint(10, 100) for _ in range(5)]
    def interact(self, individual, action):
        experience = None
        if action == 'consume':
            if self.resources:
                consumed = self.resources.pop(0)
                individual.energy += consumed
                experience = ('consume', consumed)
        return experience
class Evolution:
    def _init_(self, population_size, mutation_rate=0.1):
    ...
    def run_generation(self):
        for individual in self.population:
            individual.perceive(self.environment)
            action = individual.decide()
            experiences = self.environment.interact(individual, action)
            individual.learn(experiences)
...
class EvolutionGUI:
    def _init_(self, evolution, mayavi_widget):
        self.evolution = evolution
